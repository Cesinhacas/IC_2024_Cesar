writematrix(qTrue, 'qTrue.csv');
writematrix(q_Triad, 'q_Triad.csv');
writematrix(gyro_out, 'gyro_out.csv');
writematrix(accel_cinematica_true, 'accel.csv');
writematrix(mag_cinematica_true, 'mag.csv');
writematrix(q_Triad_sr, 'q_Triad_sr.csv');
writematrix(q_prop_sr, 'q_prop_sr.csv');
writematrix(q_est_sr, 'q_est_sr.csv');
writematrix(x_est, 'x_est.csv');
writematrix(x_prop, 'x_prop.csv');
writematrix(P_est, 'P_est.csv');
writematrix(PT_est, 'PT_est.csv');
writematrix(x_prop, 'x_prop.csv');
writematrix(PT_prop, 'PT_prop.csv');
writematrix(accel_cinematica_ruido, 'accel_ruido.csv');
writematrix(mag_cinematica_ruido, 'mag_ruido.csv');
writematrix(accel_corrigido, 'accel_corrigido.csv');
writematrix(mag_corrigido, 'mag_corrigido.csv');
writematrix(accel_cinematica_ruido(1:1201,:)', 'accel.txt');
writematrix(mag_cinematica_ruido(1:1201,:)', 'mag.txt');
writematrix(accel_corrigido(1:1201,:)', 'accel_corrigido.txt');
writematrix(mag_corrigido(1:1201,:)', 'mag_corrigido.txt');
