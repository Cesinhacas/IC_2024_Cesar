...
cd ..\
estados_est_c = readmatrix("estados_estimados_c_completo.csv");
covT_est_c = readmatrix("covT_triad_est_c_completo.csv");
cov_est_c = readmatrix("cov_triad_est_c_completo.csv");

estados_prop_c = readmatrix("estados_propagados_c_completo.csv");
covT_prop_c = readmatrix("cov_triad_prop_c_completo.csv");
cd ..\
...
hold on
grid on
title("Estimação dos estados em C")
xlim([0,1201])
set(gca,'fontsize', 17)
...
figure(3)
plot(exe, estados_est_c(:,1:4))
hold on
grid on
title("Estimação dos estados em C - apenas quatérnion")
xlim([0,1201])
set(gca,'fontsize', 17)

figure(4)
plot(exe, estados_est_mat(:,1:4))
hold on
grid on
title("Estimação dos estados em Matlab - apenas quatérnion")
xlim([0,1201])
set(gca,'fontsize', 17)
