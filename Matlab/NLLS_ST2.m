...
function [Time, passo, p] = NLLS_ST2(Dados_Corrompido, p0, Be)
passo = 1;
loop = 1;
...
while loop == 1
    ...
    sin_rho = rho - (rho^3)/6 + (rho^5)/120;
    cos_rho = 1 - (rho^2)/2 + (rho^4)/24;
    sin_phi = phi - (phi^3)/6 + (phi^5)/120;
    cos_phi = 1 - (phi^2)/2 + (phi^4)/24;
    sin_lambida = lambida - (lambida^3)/6 + (lambida^5)/120;
    cos_lambida = 1 - (lambida^2)/2 + (lambida^4)/24;
