...
{
  extern unsigned int
    sf_Tetraedro_model_sem_sim_com_FK_heur_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_Tetraedro_model_sem_sim_com_FK_heur_process_check_sum_call(nlhs,plhs,
       nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_Tetraedro_model_sem_sim_com_FK_heur_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "Tetraedro_model_sem_sim_com_FK_heur") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_Tetraedro_model_sem_sim_com_FK_heur_autoinheritance_info(nlhs,plhs,
      3,newRhs);
  }
...
{
  extern unsigned int
    sf_Tetraedro_model_sem_sim_com_FK_heur_third_party_uses_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "Tetraedro_model_sem_sim_com_FK_heur") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_Tetraedro_model_sem_sim_com_FK_heur_third_party_uses_info(nlhs,
      plhs,3,newRhs);
  }
  ...
  plhs[], int nrhs, const mxArray * prhs[] )
{
  ...
  if (strcmp(machineName, "Tetraedro_model_sem_sim_com_FK_heur") == 0) {
    ...
    newRhs[1] = prhs[2];
    return
      sf_Tetraedro_model_sem_sim_com_FK_heur_get_eml_resolved_functions_info
      (nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_Tetraedro_model_sem_sim_com_FK_heur_method_dispatcher
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"Tetraedro_model_sem_sim_com_FK_heur")) {
    return(sf_Tetraedro_model_sem_sim_com_FK_heur_method_dispatcher(simstructPtr,
...
extern void Tetraedro_model_sem_sim_com_FK_heur_terminator(void);
void sf_machine_global_terminator(void)
{
  sfGlobalMdlStartCallCounts--;
  if (sfGlobalMdlStartCallCounts == 0) {
    Tetraedro_model_sem_sim_com_FK_heur_terminator();
...
extern void Tetraedro_model_sem_sim_com_FK_heur_initializer(void);
extern void Tetraedro_model_sem_sim_com_FK_heur_register_exported_symbols
  (SimStruct* S);
extern void Tetraedro_model_sem_sim_com_FK_heur_debug_initialize(struct
  SfDebugInstanceStruct*);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  Tetraedro_model_sem_sim_com_FK_heur_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfGlobalMdlStartCallCounts == 1) {
    ...
    if (!simModeIsRTWGen) {
      Tetraedro_model_sem_sim_com_FK_heur_debug_initialize
        (sfGlobalDebugInstanceStruct);
    }

    Tetraedro_model_sem_sim_com_FK_heur_initializer();
  }
