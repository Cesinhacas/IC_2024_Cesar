...
#include "propagador_FK_terminate.h"
 ...
 * Return Type  : void
 */
void main_propagador_FK(void)
{
  double PT_est_tmp[36];
  double PT_prop[36];
  double dv1[7];
  double x_prop[7];
  double dv[3];
  double Dt_tmp;
  double i_out;
  /* Initialize function 'propagador_FK' input arguments. */
  /* Initialize function input argument 'u_gyro'. */
  Dt_tmp = argInit_real_T();
  /* Initialize function input argument 'PT_est'. */
  argInit_6x6_real_T(PT_est_tmp);
  /* Initialize function input argument 'x_est'. */
  /* Initialize function input argument 'Q'. */
  /* Call the entry-point 'propagador_FK'. */
  argInit_1x3_real_T(dv);
  argInit_7x1_real_T(dv1);
  propagador_FK(dv, Dt_tmp, PT_est_tmp, dv1, PT_est_tmp, Dt_tmp, x_prop,
                PT_prop, &i_out);
}
