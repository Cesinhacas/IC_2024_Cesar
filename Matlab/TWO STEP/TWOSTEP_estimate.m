...
sigma_sq_k=4*sum(((eye(3)+D0)*B-b0*ones(1,k_samples)).*...
    (Sigma_noise.*((eye(3)+D0)*B-b0*ones(1,k_samples))),1)...
    +5*(sum(Sigma_noise.^2,1))-(sum(Sigma_noise,1).^2);
...
theta_0_tilde=...
    I_fishinv_tilde*(L_k_tilde*((z_k_tilde-mu_k_tilde)'./sigma_sq_k'));
...
while (TS_err>stop_tol && n<max_n)
    ...
    dJdThetap_bar=(-(1/sigma_sq_bar)*(L_bar' - dbsqdtheta_p)*...
        (z_bar -L_bar'*theta_n +c'*((eye(3)+E)\c) - mu_bar))';
    ...
    % Calculate Error
    TS_err=(theta_np1-theta_n)'*...
        (I_fisher_tilde+I_fisher_bar)*...
        (theta_np1-theta_n);
